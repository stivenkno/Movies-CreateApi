...
router.get('/', async (req, res) => {
  ...
  try {
    const { data } = await movieApiInstanceAxios.get(`/search/movie?${params.toString()}`)

    if (genre && data.total_results > 0) {
      const { data: genres } = await movieApiInstanceAxios.get('/genre/movie/list')
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const { data } = await movieApiInstanceAxios.get(`/movie/${id}`)
    res.send(data)
  } catch (error) {
    res.status(error.response.status).send({ error: error.response.data.status_message })
  }
